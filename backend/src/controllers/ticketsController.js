...
const create = async (req, res) => {
  ...
  try {
    ...
    const tt = ttRows[0];
    if (!tt) { await conn.rollback(); return res.status(404).json({ error: 'Tipo de entrada no encontrado' }); }
    if (tt.sold_count >= tt.total_quota) { await conn.rollback(); return res.status(409).json({ error: 'Sin cupo disponible' }); }
    ...
    if (isPaid) {
      await conn.execute(
        `INSERT INTO payments (id, ticket_id, method, amount, status, external_id)
         VALUES (?,?,'efectivo',?,'aprobado',?)`,
        [uuidv4(), ticketId, price, payment_ref || null]
      );
      // Incrementar sold_count manualmente (el trigger lo hace en UPDATE, no en INSERT)
      await conn.execute(
    ...
    const [rows] = await conn.execute('SELECT * FROM tickets WHERE id = ?', [ticketId]);
    const ticket = rows[0];
    ticket.qr_image = qrBase64;
