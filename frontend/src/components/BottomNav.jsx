...
const BottomNav = () => {
  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 z-40 bg-slate-900/95 backdrop-blur-lg border-t border-slate-800 safe-area-bottom">
        ...
        {items.map(item => {
          ...
          return (
            <NavLink
              key={item.to}
              to={item.to}
              className={`relative flex flex-col items-center gap-0.5 px-3 py-2 rounded-lg text-xs font-medium transition-all min-w-[56px] ${
            ...
            >
              ...
              {isActive && (
                <span className="absolute -top-px left-1/2 -translate-x-1/2 w-6 h-0.5 bg-brand rounded-full" />
              )}
            </NavLink>
          );
        })}
